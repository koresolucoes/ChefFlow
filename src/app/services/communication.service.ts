...
export class CommunicationService {
  ...
  async loadAnnouncements() {
    ...
    try {
      ...
      this.error.set(error.error?.error || 'Failed to load announcements');
    } finally {
  ...
  async addAnnouncement(announcement: Partial<Announcement>) {
    ...
    try {
      ...
      this.error.set(error.error?.error || 'Failed to add announcement');
      throw err;
    } finally {
      this.loading.set(false);
    }
  ...
  async removeAnnouncement(id: string) {
    ...
    try {
      ...
      this.error.set(error.error?.error || 'Failed to remove announcement');
      throw err;
    } finally {
      this.loading.set(false);
    }
